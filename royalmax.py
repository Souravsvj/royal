...
OWNER_USERNAME = "SOURAVISLIVE"
ADMIN_IDS = ["NEO_BLADE12"]  # Add admin usernames here
ALLOWED_GROUP_IDS = [-1002215390021]
MAX_THREADS = 1000
MAX_DURATION = 300
...
def create_main_keyboard(message=None):
    ...
    if message:
        if is_owner(message.from_user):
            admin_buttons = [
                telebot.types.KeyboardButton("📄 𝗩𝗜𝗘𝗪 𝗟𝗢𝗚𝗦"),
...
def welcome(message):
    ...
    # Force show all user options immediately
    markup = create_main_keyboard(message)

    intro = "👋 *Welcome to the Bot Panel!*"

if is_owner(user):
    intro = "🛡️ *Admin Center Accessed*\nHere are your options:"
    markup = create_main_keyboard(message)
...
def delete_key_start(message):
    ...
    safe_reply_to(message, 
        ...
        "𝗙𝗼𝗿𝗺𝗮𝘁: <𝗸𝗲𝘆>\n"
        "𝗘𝘅𝗮𝗺𝗽𝗹𝗲: SOURAVISLIVE-1H-ABC123",
        parse_mode="Markdown")
...
@bot.message_handler(func=lambda msg: msg.text == "📝 𝗙𝗘𝗘𝗗𝗕𝗔𝗖𝗞")
def request_feedback(message):
    safe_reply_to(message, "📝 Please share your feedback! We'd love to hear from you.")
    bot.register_next_step_handler(message, process_feedback)

def process_feedback(message):
    user_feedback[str(message.from_user.id)] = message.text
    safe_reply_to(message, "✅ Thank you for your feedback!")
...
def attack_start(message):
    if str(message.from_user.id) not in user_feedback:
        safe_reply_to(message, "❗ *Please provide feedback before launching an attack.*"

📝 Tap below and share your thoughts using the Feedback button.", parse_mode="Markdown")
        return
...
def handle_notice_confirmation(call):
    ...
    def send_notice(chat_id):
        try:
            caption = f"»»—— SOURAVISLIVE ♥ OFFICIAL NOTICE \n\n{notice['content']}" if notice['content'] else "---------------------"
...
def generate_referral(message):
    ...
    if user_id in REFERRAL_CODES:
        ...
        code = f"SOURAVISLIVE-{user_id[:4]}-{os.urandom(2).hex().upper()}"
...
if __name__ == '__main__':
    load_data()
    load_admins()
    print("𝗕𝗼𝘁 𝗵𝗮𝘀 𝗯𝗲𝗲𝗻 𝗹𝗮𝘂𝗻𝗰𝗵𝗲𝗱 𝘀𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆! »»—— SOURAVISLIVE ♥")
    bot.polling(none_stop=True)

@bot.message_handler(func=lambda msg: msg.text == "📄 𝗩𝗜𝗘𝗪 𝗟𝗢𝗚𝗦")
def view_logs(message):
    if not is_owner(message.from_user):
        safe_reply_to(message, "⛔ You are not authorized to view logs.")
        return
    try:
        if os.path.exists("log.txt"):
            with open("log.txt", "r") as f:
                logs = f.read()[-4000:]  # last portion of logs
            safe_send_message(message.chat.id, f"🧾 *System Logs:*

<code>{logs}</code>", parse_mode="HTML")
        else:
            safe_reply_to(message, "ℹ️ No logs found.")
    except Exception as e:
        safe_reply_to(message, f"❌ Error reading logs: {e}")